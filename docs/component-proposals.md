...
- **Status:** shipped (2026-09-23). Approved by the user as proposed. Built as `WikiChain.vue` (+ `components/wiki/ChainSequence.vue`, `utils/chain.ts`), documented in `docs/wiki-components.md`. Additions to the sketch: a `note` line and `cueApprox` on each step, a `label` prop that overrides the kind's kicker (used for "Lineage" and "Chain of ownership"), labelled fork lanes that sit side by side where there is room, and a vertical "rejoin" arrow for the step after a fork. Retrofitted into Coyame and Kecksburg (the object's custody), Del Rio (the Sandia thread), McCandlish (how the story travelled), Northrop Grumman (audit → SAPOC reorganization and TRW settlement), Whistleblowers Vol.1 (Eisenhower losing control), Hidden Wing (AFMC's lineage, forward in time with a fork and a rejoin), SAIC (DSAI's seven-step trail) and Sandia (AFSWP → DTRA), with every cue read from the live captions. Left as `::wiki-org-chart`: Sarbacher (four independent lines of transmission that branch again, a tree rather than a light chain) and every true hierarchy in those articles.
...
- **Status:** shipped (2026-09-23). Approved by the user as proposed. Built as `WikiClaim.vue` (+ `utils/claim.ts`, unit-tested in `claim.test.ts`), documented in `docs/wiki-components.md`. Additions to the sketch: a `claims:` list so one block holds several claims, each grouped with its own replies (the sketch's `claim:` + `responses:` shorthand still works); optional `title`, `date`, `where` and `note` on a claim and `date` on a response; `by` may list several speakers; a `term` prop for the claim tag ("Objection", "Explanation", "Charge", "Criticism", "Reading"). Stance is a word plus a glyph, with tone only reinforcing it; the host's tone is `--graph-cat-events`, since `--graph-cat-videos` fails 3:1 as a mark in light and sepia. Retrofitted, with every cue read from the live captions: Herrera (AARO's report and Nathan's claim), MJ-12 (the six objections), Corso (the case against him; his favourable record stays a list), TR-3B (the case against Fouché), Aztec (the doodlebug trial, and the IPU "mail room" reading), Kecksburg (the four explanations), USO (the four Trepang readings) and Coyame (the soldiers' deaths). Left as panels: Edwards' Blackjack reversal (two sources that disagree on a fact, with neither answering the other; a `::wiki-compare` would suit it better), the USO Case Book's two light-wheel hypotheses (nobody answers either), and every genuinely parallel grid in those articles (witness accounts, craft types, insider allegations, the Blue Berets).
...
- **Status:** shipped (2026-09-23). Approved by the user as proposed. Built as `WikiMap.vue` (+ `utils/map.ts`, unit-tested in `map.test.ts`), documented in `docs/wiki-components.md`. One runtime dependency, `d3-geo` (projection, clipping, circles). The outlines are Natural Earth (1:50m Americas and lakes, 1:110m elsewhere, 1:10m US state borders), built by `app/scripts/build-map-outlines.mjs` into two small same-origin files in `app/public/geo/`: no tile server and no third-party request. Location pages carry `coordinates: [lat, lon]` frontmatter, baked at build time and returned on refs by `/api/resolve`. Changes from the sketch: route stops are pin numbers as shown on the map (1-based) or pin names, and a route can be dashed and labelled; pins take an optional on-map `label`, a `radius` in miles (a dashed circle) and inline `coordinates` (for places with no page, or a point within a larger place); regional maps label the countries in view and carry a scale bar. Retrofitted into Kecksburg (the fireball's reported course and the object's route to Lockbourne and Wright-Patterson), Coyame (the Fort Bliss–Presidio–Coyame–Davis Mountains–Van Horn recovery loop), Peru (Lima, Pucallpa and Iquitos with the host's 90-mile radius), Del Rio (the two sites on either side of the town), 29 Palms (the north-west departure toward R2508) and DUMBs (the Antelope Valley network's alleged links). Left alone: Dugway (everything happens inside one installation, and the only precise site the video discusses is an alleged tunnel entrance the host asks viewers not to seek out), the US Navy video (its storage sites are scattered one-off claims, not a route or a cluster; the panels already carry the place names) and Crane (not yet enriched).
